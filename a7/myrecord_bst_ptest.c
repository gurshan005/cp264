...
BSTDS bstds = { 0 };  // global BSTDS; all fields initialize to zero.
...
void display_bst_stats(BSTDS *bstdsp) {
    printf("count: %d ", bstdsp->count);
    printf("mean: %.1f ", bstdsp->mean);
    printf("stddev: %.1f", bstdsp->stddev);
    printf("\n");
}
